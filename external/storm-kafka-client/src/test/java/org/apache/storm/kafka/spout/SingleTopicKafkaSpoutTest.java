...
public class SingleTopicKafkaSpoutTest {
    ...
    void populateTopicData(int msgCount) throws InterruptedException, ExecutionException, TimeoutException {
        kafkaUnitRule.getKafkaUnit().createTopic(SingleTopicKafkaSpoutConfiguration.TOPIC);

        for (int i = 0; i < msgCount; i++) {
            ProducerRecord<String, String> producerRecord = new ProducerRecord<>(
                SingleTopicKafkaSpoutConfiguration.TOPIC, Integer.toString(i),
                Integer.toString(i));
            kafkaUnitRule.getKafkaUnit().sendMessage(producerRecord);
        }
    }

    private void initializeSpout() throws InterruptedException, ExecutionException, TimeoutException {
    ...
    public void testShouldContinueWithSlowDoubleAcks() throws Exception {
        try (SimulatedTime simulatedTime = new SimulatedTime()) {
            int messageCount = 20;
            populateTopicData(messageCount);
            initializeSpout();
    ...
    public void testSpoutMustRefreshPartitionsEvenIfNotPolling() throws Exception {
        try (SimulatedTime time = new SimulatedTime()) {
            ...
            spout.nextTuple();
            verify(collector, never()).emit(anyObject(), anyObject(), anyList());
            ...
            spout.nextTuple();
            verify(collector).emit(anyObject(), anyObject(), anyObject());
        }
    }
}
